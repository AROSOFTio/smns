<?php
/**
 * One-time cleanup for semester-mismatched course registrations.
 *
 * Rules:
 * - A registration is "mismatched" when course.semester_offered is neither
 *   the semester's semester_number nor 3 (both semesters).
 * - If a mismatched registration has at least one result row, it is preserved.
 * - If a mismatched registration has no result row, it is archived then deleted.
 *
 * Usage:
 *   php scripts/cleanup_mixed_course_registrations.php            # dry run
 *   php scripts/cleanup_mixed_course_registrations.php --apply    # execute cleanup
 */
...
$batchId = 'mixed-sem-cleanup-' . date('Ymd-His');
...
try {
    ...
    $sql = "
        ...
        WHERE c.semester_offered <> 3
          AND c.semester_offered <> s.semester_number
        ORDER BY cr.id ASC
    ";
    ...
    echo "Batch: {$batchId}\n";
    echo "Total mismatched registrations: " . count($rows) . "\n";
    ...
    if (!empty($toPreserve)) {
        ...
        $sample = array_slice($toPreserve, 0, 10);
        foreach ($sample as $r) {
            echo "  id={$r['id']} student={$r['student_id']} course={$r['course_code']} sem_id={$r['semester_id']} sem_offer={$r['semester_offered']} sem_num={$r['semester_number']} results={$r['result_count']}\n";
        }
    }

    if (!empty($toDelete)) {
        echo "\nSample delete rows (first 10):\n";
        $sample = array_slice($toDelete, 0, 10);
        foreach ($sample as $r) {
            echo "  id={$r['id']} student={$r['student_id']} course={$r['course_code']} sem_id={$r['semester_id']} sem_offer={$r['semester_offered']} sem_num={$r['semester_number']}\n";
        }
    }
    ...
    // Resolve course_registrations base columns dynamically for schema compatibility.
    $baseCols = [];
    ...
    $archiveParams = array_merge([$batchId, 'mixed_semester_without_results'], $deleteIds);
