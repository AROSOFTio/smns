...
$unifiedLoginQuery['role'] = 'lecturer';
    ...
    if ($step === 1) {
        ...
        if (empty($entered_username)) {
            ...
            if (!empty($result['mfa_required'])) {
                header('Location: ' . BASE_URL . '/views/auth/mfa-verify.php?module=lecturer');
                exit;
            } elseif (!empty($result['consent_required'])) {
                header('Location: ' . BASE_URL . '/views/auth/privacy-consent.php?module=lecturer');
                exit;
            } elseif ($result['success'] && $result['role'] === 'lecturer') {
            ...
            <form method="POST" action="" class="login-form" autocomplete="on">
                ...
                <?php elseif ($step === 2): ?>
                    ...
                    <div class="text-center mt-2">
                        <a href="<?php echo e(BASE_URL . '/views/auth/forgot-password.php?role=lecturer'); ?>" class="btn btn-link btn-sm">
                            Forgot password?
                        </a>
                    </div>
