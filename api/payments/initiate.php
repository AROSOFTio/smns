...
$msisdn = trim((string)($_POST['msisdn'] ?? ($_POST['phone'] ?? ($jsonBody['msisdn'] ?? ($jsonBody['phone'] ?? '')))));
$provider = trim((string)($_POST['provider'] ?? ($jsonBody['provider'] ?? '')));
...
if ($referenceNumber === '' || $msisdn === '') {
    http_response_code(422);
    echo json_encode(['success' => false, 'message' => 'PRN and phone number are required.']);
    exit;
}
...
try {
    ...
    $result = $service->initiatePaymentByReference($studentId, $referenceNumber, $msisdn, $provider);
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Unable to initiate mobile money payment.',
